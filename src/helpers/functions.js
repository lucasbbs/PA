...
export const getDataForTheFirstChart = (
  ...
  const initialSlice = isInitialUndefined
    ...
    : datesSet.indexOf(firstPeriod) - 1;
  ...
  datesSet.forEach((el) => {
    labels.push(
      ...new Set(
        el.map((date) =>
          format(new Date(date[0]), 'MMM/yyyy', { locale: ptBR })
        )
      )
    );
    values.push(
...
export const getDataForTheInflationChart = (
  ...
  let inflations = inflation.slice(
    isInitialUndefined
      ...
      : inflation.map((e) => e.data).indexOf(firstPeriod) - 1,
  ...
  inflations.forEach((e) => {
    labels.push(format(new Date(e.data + 1), 'MMM/yyyy', { locale: ptBR }));
    values.push(e.valor);
  });
  return [values, labels];
};
...
export const getDataForTheTopInvestmentsTable = (investments) => {
  ...
  return returns.sort((a, b) => b[1] - a[1]).slice(0, 7);
};
